...
client_functions=$(grep -E '{..' $script_path/../client.c | cut -d" " -f6 | sed -r 's/"//g' | sed -r 's/,//g' | sort)
man_functions=$(grep -E '\\fB' $script_path/../berryc.1 | sed -r 's/\\fB//g' | sed -r 's/(\\fR.*)//g' | sed -r 's/\[//g' | sed -r 's/\]//g' | tr "|" '\n' | sort)
